...
#New_f
def New_values_detector(Ddetector, fknee_select, Net_select):
    a = 0
    v = 0
...
def Simulated_noise(Ddetector, fknee_select, Net_select):
    new_fknee = 0
    new_NET = 0
    new_fknee, new_NET = New_values_detector(Ddetector, fknee_select, Net_select)
    # AnalyticNoise function
    # This generates an analytic PSD for a set of detectors, given input values for the knee frequency,
    # NET, exponent, sample rate, minimum frequency, etc.
    PSD_Analytic_result = AnalyticNoise(rate=Ddetector.detrate, fmin=Ddetector.detfmin, detectors=Ddetector.detnames,
                              fknee=new_fknee, alpha=Ddetector.detalpha, NET=new_NET)
    return PSD_Analytic_result
...
#
def TOD_generator(Ddetector, Dschedule, noise_ar, weather_atacama, focalplane, MAPA_SIM, Dsimulation, outdirD, outprefixD):
    # communication MPI
    data = 0
